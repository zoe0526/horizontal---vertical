...
namespace UnityEditor.UI
{
    ...
    /// </summary>
    #if UNITY_EDITOR
	[CustomEditor(typeof(CanvasScalerExt), true)]
    [CanEditMultipleObjects]
    public class CanvasScalerExtEditor : Editor
    {
        SerializedProperty m_UiScaleMode;
        SerializedProperty m_WorldCamera;
		SerializedProperty m_UiWorldSpaceScaleMode;
        ...
        private class Styles
        {
            ...
            public GUIContent worldCamera=null;
			public GUIContent uiWorldSpaceScaleModeContent=null;
        ...
        protected virtual void OnEnable()
        {
			...
			m_UiWorldSpaceScaleMode = serializedObject.FindProperty("m_UiWorldSpaceScaleMode");
            m_WorldCamera = serializedObject.FindProperty("m_WorldCamera");
        ...
        public override void OnInspectorGUI()
        {
            ...
            bool showWorld = ((target as CanvasScalerExt).GetComponent<Canvas>().renderMode == RenderMode.WorldSpace);
            for (int i = 0; i < targets.Length; i++)
            {
                CanvasScalerExt scaler = targets[i] as CanvasScalerExt;
			...
			if (showWorld || showWorldDiffers) {
                EditorGUILayout.PropertyField(m_WorldCamera, s_Styles.worldCamera);
				EditorGUILayout.PropertyField(m_UiWorldSpaceScaleMode, s_Styles.uiWorldSpaceScaleModeContent);
			}
            ...
            if (!showWorldDiffers && !(!showWorld && m_UiScaleMode.hasMultipleDifferentValues))
            {
                ...
                if (showWorld)
                {
					if (m_UiWorldSpaceScaleMode.enumValueIndex == (int)CanvasScalerExt.WorldSpaceScaleMode.DoNotUseScale) {
                    	EditorGUILayout.PropertyField(m_DynamicPixelsPerUnit);
					} else if (m_UiWorldSpaceScaleMode.enumValueIndex == (int)CanvasScalerExt.WorldSpaceScaleMode.ScaleWithScreenSize) {
						EditorGUILayout.PropertyField(m_ReferenceResolution);
						EditorGUILayout.PropertyField(m_ScreenMatchMode);
						if (m_ScreenMatchMode.enumValueIndex == (int)CanvasScalerExt.ScreenMatchMode.MatchWidthOrHeight && !m_ScreenMatchMode.hasMultipleDifferentValues)
						{
                ...
                // Constant pixel size
                else if (m_UiScaleMode.enumValueIndex == (int)CanvasScalerExt.ScaleMode.ConstantPixelSize)
                {
                    EditorGUILayout.PropertyField(m_ScaleFactor);
                }
                // Scale with screen size
                else if (m_UiScaleMode.enumValueIndex == (int)CanvasScalerExt.ScaleMode.ScaleWithScreenSize)
                {
                    EditorGUILayout.PropertyField(m_ReferenceResolution);
                    EditorGUILayout.PropertyField(m_ScreenMatchMode);
                    if (m_ScreenMatchMode.enumValueIndex == (int)CanvasScalerExt.ScreenMatchMode.MatchWidthOrHeight && !m_ScreenMatchMode.hasMultipleDifferentValues)
                ...
                else if (m_UiScaleMode.enumValueIndex == (int)CanvasScalerExt.ScaleMode.ConstantPhysicalSize)
                {
